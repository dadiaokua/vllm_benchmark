...
from util import some_endpoint_test, start_tunnel, stop_tunnel, sample_sharegpt_requests
...
async def make_request(client, output_tokens, request_timeout, request):
...
async def worker(client, semaphore, queue, results, output_tokens,
                 request_timeout, rate_lambda, distribution, sample_content):
    while True:
        if distribution == "possion":
            delay = float(np.random.poisson(rate_lambda))  # 生成泊松分布的时间间隔
        elif distribution == "normal":
            delay = max(0.0, float(np.random.normal(loc=1 / rate_lambda, scale=0.1)))  # loc 是平均值，scale 是标准差
        else:
            delay = np.random.uniform(low=0.5, high=2.0)  # 生成 [0.5, 2.0] 范围的时间间隔
        ...
        async with semaphore:
            ...
            logging.info("Prompt request: " + request)
            try:
                result = await make_request(client, output_tokens, request_timeout, request)
                if result:
                    results.append(result)
                else:
                    logging.warning(f"Request {task_id} failed")
            except Exception as e:
                logging.error(f"Request {task_id} raised an exception: {e}")
            queue.task_done()
...
async def run_benchmark(num_requests, concurrency, request_timeout, output_tokens, client, api_key, distribution, qps):
    ...
    workers = [asyncio.create_task(worker(client, semaphore, queue, results, output_tokens,
                                          request_timeout, qps, distribution, sample_content)) for _ in
               range(concurrency)]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--vllm_url", type=str, required=True, help="URL of the vLLM server",
                        default="http://222.201.144.120:8000")
